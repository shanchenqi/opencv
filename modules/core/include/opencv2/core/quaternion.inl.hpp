...
Vec<T, 3> Quat<T>::toEulerAngles( EulerAnglesType eulerAnglesType){
    ...
    if(!rotationR[eulerAnglesType][12]){
        ...
        {
            CV_LOG_WARNING(NULL,"Gimbal Lock will occur. Euler angles are non-unique, we set the third angle to 0");
        ...
        if (abs(rotationR[eulerAnglesType][0] - 1) < CV_QUAT_CONVERT_THRESHOLD)
        {
            CV_LOG_WARNING(NULL,"Gimbal Lock will occur. Euler angles are non-unique, we set the first angle to 0");
